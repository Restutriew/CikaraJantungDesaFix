...
public class EditSuratActivity extends AppCompatActivity implements View.OnClickListener {

    public static final String DATA_FORMAT_SURAT_USER = "extra_data";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SuratV2Model modelData = getIntent().getParcelableExtra(DATA_FORMAT_SURAT_USER);
        idSurat = modelData.getId();
        kodeSurat = modelData.getKode();
        namaSurat = modelData.getNama_surat();

        sessionManager = new SessionManager(EditSuratActivity.this);
        ...
        loadingDialog = new LoadingDialog(EditSuratActivity.this);
    ...
    private void addAjuanSuratEdit() {
        loadingDialog.startLoading();
        ...
        String URL_TAMBAHADUAN = link + "updatesurat";
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.PATCH, URL_TAMBAHADUAN,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            if (success.equals("1")) {
                                Toast.makeText(EditSuratActivity.this, "Pengajuan Surat Sukses", Toast.LENGTH_LONG).show();
                                finish();

                            } else {
                                Toast.makeText(EditSuratActivity.this, "Pengajuan Gagal! Anda Telah Melakukan Pengajuan Untuk Data Ini", Toast.LENGTH_LONG).show();
                            }
                            loadingDialog.dissmissDialog();

                        } catch (JSONException e) {
                            e.printStackTrace();
                            Toast.makeText(EditSuratActivity.this, "Pengajuan Gagal!" + e.toString(), Toast.LENGTH_LONG).show();
                            loadingDialog.dissmissDialog();
                        }
                    }
                },
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(EditSuratActivity.this, "Pengajuan Gagal! : Cek Koneksi Anda" + error.toString(), Toast.LENGTH_LONG).show();
                        loadingDialog.dissmissDialog();
                    }
                }) {
            @Override
